	...
	while(1)
	{
		...
		if( FD_ISSET(server.listen_fd, &readfds) ) {
			...
			if ( conn_fd < 0 ) {/*{{{*/
				...
				fprintf( stderr, "\nOK: Accepting a new request... fd %d from %s\n", conn_fd, requestP[conn_fd].host );
			}
...
// fprintf( stderr, "QUERY = %s\n", requestP[i].query );
					...
					int slen = strlen(requestP[i].file);
					for(int j = 0; j < slen; ++j)
						if( ( !isalnum(requestP[i].file[j]) && requestP[i].file[j] != '_' )
								|| ( !isalnum(requestP[i].file[j]) && requestP[i].file[j] != '_') ) {
							// TODO: print 400
							fprintf( stderr, "ERROR: 400, not [a-z][A-Z][0-9][_] type\n" );
						}
					...
					pipe(pout);
					for(int j = 0; j < 2; ++j) {
						requestP[i].pin[j]  = pin[j];
						requestP[i].pout[j] = pout[j];
					}

					pid_t pid = fork();
					if(pid == 0) {	// child
						dup2( requestP[i].pout[0] , 0);
						dup2( requestP[i].pin[1]  , 1);
						close( requestP[i].pout[0] );
						close( requestP[i].pin[1]  );
						char *fn = requestP[i].file;
						char pth[1028] = "./";
						strncat(pth, fn, strlen(fn));
						execl(pth, fn, NULL);
					}	// else	-> parent

					// TODO: check if the filename is correct
					// write filename to file_reader
					char *qu = &(requestP[i].query[9]);
					write(requestP[i].pout[1] , qu, strlen(qu));
					write(requestP[i].pout[1], "\n", 2);
		...
		if( requestP[i].pin[0] >= 0 && FD_ISSET(requestP[i].pin[0], &readfds) ) {
				...
				// read buf from file_reader & write to client/*{{{*/
				char buf[MAXBUFSIZE];
				...
				if( (len = read(requestP[i].pin[0], buf, MAXBUFSIZE)) > 0) {
					...
					fprintf( stderr, "OK: WRITING %ld bytes to request fd %d\n" , len, requestP[i].conn_fd );
					nwritten = write( requestP[i].conn_fd, buf, len);
					fprintf( stderr, "OK: COMPLETE writing %d bytes to request fd %d\n", nwritten, requestP[i].conn_fd );
