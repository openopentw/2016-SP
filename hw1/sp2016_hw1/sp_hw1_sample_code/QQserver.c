...
typedef struct {/*{{{*/
	...
	int file_fd;	// fd to the file that we open for reading or writing
	// created by me
...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		while(select(maxfd + 1, &readfds, &writefds, NULL, &timeout) <= 0) {;}
		...
		for(int i = svr.listen_fd + 1; i < maxfd; ++i) {		// i 的最大值不確定要不要 + 1
			if( FD_ISSET(i, &readfds) ) {
				...
				if (ret < 0) {/*{{{*/
					...
					continue;		// Remember that this was to the "while(1)" originally
				}/*}}}*/
				// requestP[i]->filename is guaranteed to be successfully set.
				// if (file_fd == -1) {
				if (i == conn_fd) {		// new connectin
					// open the file here.
